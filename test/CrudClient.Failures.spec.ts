...
describe('CRUD Client (Failures)', () => {
	...
	it('should throw if requested item index does not exist anymore.', async () => {
		...
		expect(() => (client as any).getItemIndex(item, index)).toThrowError() // eslint-disable-line @typescript-eslint/no-explicit-any
	})
	...
	it('should forward exception to console.error', () => {
		...
		(client as any).forwardError(msg) // eslint-disable-line @typescript-eslint/no-explicit-any
	...
	it('should forward exception throw', () => {
		...
		const msg = 'Some test error'
		expect(() => (client as any).forwardError(msg)).toThrow() // eslint-disable-line @typescript-eslint/no-explicit-any
	})
})
